...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place != hive:
            ...
            if bee != None and self.min_range <= cnt <= self.max_range:
                return bee
...
class WallAnt(Ant):
    ...
    implemented = True
    def __init__(self):
        super().__init__(self.armor)      #调用父类的构造函数设置护甲
# END Problem 8
...
class NinjaAnt(Ant):
    ...
    food_cost = 5
    armor = 1
    blocks_path = False     #does not block the path
    # END Problem 7
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        bee.reduce_armor(bee.armor)     # reducing the Bee's armor to 0 
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        def double(ant):
            if not ant:
                return 
            if ant not in self.sub_ants:
                ant.damage *= 2
                self.sub_ants.add(ant)
            if ant.container:
                double(ant.ant)
