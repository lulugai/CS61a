...
def is_swap(score0, score1):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if score0 <= 1 or score1 <= 1: return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    h = both(announce_highest(0), both(announce_highest(1), announce_lead_changes()))
    while True:
        if score0 >= goal or score1 >= goal:
            break
        if player == 0: 
            num_rolls = strategy0(score0, score1)
            gain = take_turn(num_rolls, score1, dice)
            score0 += gain
        elif player == 1:
            num_rolls = strategy1(score1, score0)
            gain = take_turn(num_rolls, score0, dice)
            score1 += gain 
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if gain > previous_high:
            new_high = gain
            print("{} {}! That's the biggest gain yet for Player {}".format(gain, 'point' if gain == 1 else 'points', who))
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    ans = 1
    for i in range(1, 11):
        averaged_score = make_averaged(roll_dice, num_samples)
        tmp = averaged_score(i, dice)
        if tmp > max_score:
            max_score = tmp
            ans = i
    return ans
